...
console.log("Tu personaje " + personaje.name + " tiene un Carisma igual a " + personaje.cha)

// ---------Constructores
//Sintaxis
function Personaje(id, name, cla, lvl, background, str, dex, con, int, wis, cha) {
    this.id = id;
    this.name = name;
    this.cla = cla;
    this.lvl = lvl;
    ...
    this.describir = function() {
        console.log("¡Hola extraño! Mi nombre es " + this.name + " y soy un " + this.cla + " " + this.background + " de nivel " + this.lvl + ".")
    }
}

//Nuevas instancias
const personaje1 = new Personaje(02, "Verminius", "Barbaro", 4, "Soldado", 19, 14, 16, 11, 10, 8);
console.log(personaje1)

const personaje2 = new Personaje(03, "Baltazar", "Bardo", 6, "Artista", 10, 16, 18, 8, 12, 20);
console.log(personaje2)

const personaje3 = new Personaje(04, "Nathaniel", "Paladin", 4, "Embrujado", 20, 14, 16, 9, 10, 16);
...
for (const prop in personaje1) {
    console.log(prop, personaje1[prop])
}
...
class Weapon {
    ...
    //Métodos
    presentar() {
        console.log("Esta es la poderosa " + this.name + " una " + this.weaponType + " capás de hacer un daño de " + this.dmg + " de daño " + this.dmgType + ".")
    }
    ...
    dañar() {
        console.log((Math.floor(Math.random() * 8 + 1)) + (Math.floor(Math.random() * 8 + 1)))
    }
}
